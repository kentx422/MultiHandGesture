	...
	String someProcessForDecember(String message){
		...
		for(int i=0; i<splitSemicolon.length;i++){
			...
			if(operator.equals("connect")){
			    ...
			    ChatServerForSOMG4DUntillDecember.answerOrder = new ArrayList<String>();
			    ChatServerForSOMG4DUntillDecember.orderList = new ArrayList<String>();

				long diffTime = System.currentTimeMillis() - ChatServerForSOMG4DUntillDecember.startTime;
...
//				for(int j=0;j<ChatServerForSOMG4DUntillDecember.requestOrder.size();j++){
//					messageForWriting += ChatServerForSOMG4DUntillDecember.requestOrder.get(j)+","+ChatServerForSOMG4DUntillDecember.answerOrder.get(j)+"\n";
//				}
...
//				writeCSV(filename, messageForWriting);
				...
				ChatServerForSOMG4DUntillDecember.orderList = new ArrayList<>(Arrays.asList(
						"ABCD","ABDC","ACBD","ACDB","ADBC","ADCB",
						"BACD","BADC","BCAD","BCDA","BDAC","BDCA",
						"CABD","CADB","CBAD","CBDA","CDAB","CDBA",
						"DABC","DACB","DBAC","DBCA","DCAB","DCBA"));				
				if(ChatServerForSOMG4DUntillDecember.orderList.size()!=0){
					int temp =(int) (Math.random()*ChatServerForSOMG4DUntillDecember.orderList.size());
					String tempOrder = ChatServerForSOMG4DUntillDecember.orderList.get(temp);
					ChatServerForSOMG4DUntillDecember.orderList.remove(temp);
					ChatServerForSOMG4DUntillDecember.requestOrder.add(tempOrder);
				...
				if(ChatServerForSOMG4DUntillDecember.orderList.size()!=0){
					...
					ChatServerForSOMG4DUntillDecember.orderList.remove(temp);
					ChatServerForSOMG4DUntillDecember.requestOrder.add(tempOrder);
					sendMessageAll(udid+",replyNextOrder,"+tempOrder+","+orderButtonFlag);
					
					String tempAnswerOrder = "";
					while(ChatServerForSOMG4DUntillDecember.tempAnswerOrder.size()!=0){
						int tempNum = 0;
						long timeMin = 999999999;
						for(int j=0;j<ChatServerForSOMG4DUntillDecember.tempAnswerOrder.size();j++){
							if(timeMin > Long.parseLong(ChatServerForSOMG4DUntillDecember.tempTimeOfAnswerOrdr.get(j))){
								timeMin = Long.parseLong(ChatServerForSOMG4DUntillDecember.tempTimeOfAnswerOrdr.get(j));
								tempNum = j;
							}
						}
						tempAnswerOrder += ChatServerForSOMG4DUntillDecember.tempAnswerOrder.get(tempNum);
						ChatServerForSOMG4DUntillDecember.tempAnswerOrder.remove(tempNum);
					}
					ChatServerForSOMG4DUntillDecember.answerOrder.add(tempAnswerOrder);
				}
				else{
				...
				//pathを変更
				ChatServerForSOMG4DUntillDecember.path = System.getProperty("user.dir")+"\\result\\"+getNowTime();

			}
			else if(operator.equals("setTime")){
				long diffTime = System.currentTimeMillis() - ChatServerForSOMG4DUntillDecember.startTime;
				sendMessageAll(udid+",replySetTime,"+String.valueOf(diffTime));
			}
...
//				long timeLx = Long.parseLong(splitComma[4]);
				...
				if(lxLog.size()>0){
					Double lastLx = Double.parseDouble(lxLog.get(lxLog.size()-1));
					if(Math.abs(Double.parseDouble(lx)-lastLx) > lastLx * 0.1){
						//System.out.println("in "+deviceChar);
						if(ChatServerForSOMG4DUntillDecember.tempAnswerOrder.indexOf(deviceChar)==-1){
							ChatServerForSOMG4DUntillDecember.tempAnswerOrder.add(deviceChar);
							ChatServerForSOMG4DUntillDecember.tempTimeOfAnswerOrdr.add(timeLx);
						}
					}
				...
				sendMessageAll(udid+",replyChooseGesture,"+timeGesture+","+gesture);
			}
...
//	}
	...
	// メッセージ送信
		public void sendMessage(ChatServerThreadForSOMG4DUntillDecember talker, String message) {
			try {
	...
	public void writeCSV(String filename,String message) {
		...
		try {
			...
			File f = new File(ChatServerForSOMG4DUntillDecember.path+"\\"+filename + ".csv");
	...
	public String someProcess(String str){
		...
		if(ChatServerForSOMG4DUntillDecember.ImageSomeShareFlag==1){
			long diff = startTime- ChatServerForSOMG4DUntillDecember.ImageSomeShareTimeStamp ;
	...
	public String slashProcess(String macAddress) {
		...
		// 前回がHIDEのとき
			...
			ChatServerForSOMG4DUntillDecember.ImageSomeShareImageID = device[deviceIDBuf].getImageID()[lastTimes];
			...
			return "ImageSomeShareFlagON," + ChatServerForSOMG4DUntillDecember.ImageSomeShareImageID
					+ "," + macAddress;
		}
		// 誰かがHIDE&SLASHをしているとき
		else if (ChatServerForSOMG4DUntillDecember.ImageSomeShareFlag == 1) {
			return "ImageSomeShare," + ChatServerForSOMG4DUntillDecember.ImageSomeShareImageID + ","
					+ macAddress;
		}
