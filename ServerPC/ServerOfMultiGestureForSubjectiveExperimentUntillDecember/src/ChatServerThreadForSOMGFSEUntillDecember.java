...
//チャットサーバスレッド
public class ChatServerThreadForSOMGFSEUntillDecember extends Thread {
	private static List<ChatServerThreadForSOMGFSEUntillDecember> threads = new ArrayList<ChatServerThreadForSOMGFSEUntillDecember>();// スレッド郡
	...
	// コンストラクタ
	public ChatServerThreadForSOMGFSEUntillDecember(Socket socket) {
		super();
		this.socket = socket;
		threads.add(this);
	}
	...
	String someProcessForDecember(String message){
		...
		for(int i=0; i<splitSemicolon.length;i++){
			...
			if(operator.equals("connect")){	
				...
				long diffTime = System.currentTimeMillis() - ChatServerForSOMGFSEUntillDecember.startTime;
				sendMessage(this,"server,timeFromServer,"+String.valueOf(diffTime));
				...
				File file = new File(ChatServerForSOMGFSEUntillDecember.path);
				...
				//pathを変更
				ChatServerForSOMGFSEUntillDecember.path = System.getProperty("user.dir")+"\\result\\"+getNowTime();
...
//	}
	...
	public void sendMessageAll(String message) {
		ChatServerThreadForSOMGFSEUntillDecember thread;
		System.out.println("Start of Send message all user:	" + message);
		for (int i = 0; i < threads.size(); i++) {
			thread = (ChatServerThreadForSOMGFSEUntillDecember) threads.get(i);
			if (thread.isAlive())
	...
	// メッセージ送信
		public void sendMessage(ChatServerThreadForSOMGFSEUntillDecember talker, String message) {
			try {
	...
	public void writeCSV(String filename,String message) {
		...
		try {
			...
			File f = new File(ChatServerForSOMGFSEUntillDecember.path+"\\"+filename + ".csv");
	...
	public String someProcess(String str){
		...
		if(ChatServerForSOMGFSEUntillDecember.ImageSomeShareFlag==1){
			long diff = startTime- ChatServerForSOMGFSEUntillDecember.ImageSomeShareTimeStamp ;
			if (diff>(long)(10000000)){
				ChatServerForSOMGFSEUntillDecember.ImageSomeShareFlag=0;
				ChatServerForSOMGFSEUntillDecember.ImageSomeShareImageID   = -1;
				ChatServerForSOMGFSEUntillDecember.ImageSomeShareTimeStamp = 0;
	...
	public String slashProcess(String macAddress) {
		...
		// 前回がHIDEのとき
			...
			ChatServerForSOMGFSEUntillDecember.ImageSomeShareFlag = 1;
			ChatServerForSOMGFSEUntillDecember.ImageSomeShareImageID = device[deviceIDBuf].getImageID()[lastTimes];
			ChatServerForSOMGFSEUntillDecember.ImageSomeShareTimeStamp = device[deviceIDBuf]
					.getStartTime()[lastTimes];
			System.out.println("ImageSomeShareFlag ON");
			return "ImageSomeShareFlagON," + ChatServerForSOMGFSEUntillDecember.ImageSomeShareImageID
					+ "," + macAddress;
		}
		// 誰かがHIDE&SLASHをしているとき
		else if (ChatServerForSOMGFSEUntillDecember.ImageSomeShareFlag == 1) {
			return "ImageSomeShare," + ChatServerForSOMGFSEUntillDecember.ImageSomeShareImageID + ","
					+ macAddress;
		}
