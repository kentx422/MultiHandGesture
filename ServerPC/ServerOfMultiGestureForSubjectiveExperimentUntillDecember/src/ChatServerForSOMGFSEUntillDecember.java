...
//チャットサーバ
public class ChatServerForSOMGFSEUntillDecember {
	...
	public void start(int port) {
		...
		Socket           socket;//ソケット
		ChatServerThreadForSOMGFSEUntillDecember thread;//スレッド
		...
		try {
			...
			while(true) {
				try {
					...
					//チャットサーバスレッド開始
					thread=new ChatServerThreadForSOMGFSEUntillDecember(socket);
					thread.start();
	...
	public static void main(String[] args) {
		...
		ChatServerForSOMGFSEUntillDecember server=new ChatServerForSOMGFSEUntillDecember();
		server.start(8080);
	}
