package com.example.isdl.dempappaddimageshareforjulymonthlylecture;
...
public class MainActivity extends Activity implements SensorEventListener {
    ...
    private int redImage = R.drawable.redimage;
    private int blueImage = R.drawable.blueimage;
    private int blackImage = R.drawable.blackimage;
    private int whiteImage = R.drawable.whiteimage;
    private int imageID = 0;
    private int[] imageList = {redImage, blueImage, blackImage, whiteImage};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        testImage.setImageResource(imageList[touchMode]);
...
//                    }
//                }
//                else if(touchMode==imageList.length){
...
//                }
                touchMode++;
                if(touchMode==(imageList.length))touchMode=0;
                testImage.setImageResource(imageList[touchMode]);
...
//                onServe(System.currentTimeMillis()+","+" ");
    ...
    public void someProcess(String strBuf){
        ...
        int existFlag = 0;
        int tempImageID = Integer.parseInt(splitConma[1]);
        for(int i=1;i<splitSemicolon.length;i++){
            splitConma = splitSemicolon[i].split(",");
            if(splitConma[0].equals(deviceName)&&onoff==1){
                //backNumber.setText(""+(Integer.parseInt(splitConma[1])+1));
                //relativeLayout.setBackgroundColor(Color.rgb(9,83,133));
                touchMode = tempImageID;
                testImage.setImageResource(imageList[touchMode]);
                existFlag = 1;
            }
        }
